...
Wallpappr generates each wallpaper's "friendly name" two ways. First, it checks if the file has the `kMDItemDescription` attribute set. If it does it uses that value. If it doesn't, it tries to make a friendly name from the file name. Ex: `contoso-red-waves.png` would become "Contoso Red Waves". It also works if there are spaces or underscores between words.

Why use `kMDItemDescription`? Sometimes the name you want doesn't make sense as a file name. `contoso-nyc-headquarters.png` would become Contoso Nyc Headquarters, instead of capitalizing NYC. Using non alphanumeric characters (&, #, $) would also complicate things.
